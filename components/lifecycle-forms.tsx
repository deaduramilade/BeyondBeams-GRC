...
function ActionForm({ children, action, submitLabel = "Save" }: { children: React.ReactNode; action: (data: FormData) => Promise<Result>; submitLabel?: string }) {
  ...
  return <form onSubmit={submit} className="grid gap-3">{children}<Button disabled={busy}>{busy ? <LoaderCircle className="size-4 animate-spin"/> : <Check className="size-4"/>}{submitLabel}</Button>{message && <p role="status" className={message === "Saved successfully." ? "text-xs text-emerald-600" : "text-xs text-destructive"}>{message}</p>}</form>;
}
...
export function TreatmentPlanForm({ risks }: { risks: Option[] }) {
  return <ActionForm action={(data) => createTreatment({ riskId: data.get("riskId"), strategy: data.get("strategy"), summary: data.get("summary"), targetDate: data.get("targetDate") })} submitLabel="Create treatment plan"><SelectField name="riskId" label="Risk" options={risks}/><SelectField name="strategy" label="Strategy" options={["ACCEPT", "MITIGATE", "TRANSFER", "AVOID"].map((value) => ({ value, label: value }))}/><Field name="summary" label="Measurable treatment outcome"/><Field name="targetDate" label="Target date" type="date"/></ActionForm>;
}

export function TreatmentActionForm({ plans, users }: { plans: Option[]; users: Option[] }) {
  return <ActionForm action={(data) => createTreatmentAction({ treatmentPlanId: data.get("treatmentPlanId"), title: data.get("title"), description: data.get("description"), ownerId: data.get("ownerId"), dueDate: data.get("dueDate"), priority: data.get("priority") })} submitLabel="Add treatment action"><SelectField name="treatmentPlanId" label="Treatment plan" options={plans}/><Field name="title" label="Action title"/><Field name="description" label="Required outcome"/><SelectField name="ownerId" label="Owner" options={users}/><div className="grid grid-cols-2 gap-3"><Field name="dueDate" label="Due date" type="date"/><SelectField name="priority" label="Priority" options={["LOW", "MEDIUM", "HIGH", "CRITICAL"].map((value) => ({ value, label: value }))}/></div></ActionForm>;
}

export function TreatmentDecision({ planId }: { planId: string }) {
  ...
  return <div className="grid gap-2"><Input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Decision rationale (minimum 10 characters)"/><div className="flex gap-2"><Button size="sm" disabled={reason.length < 10} onClick={() => decide("APPROVED")}>Approve</Button><Button size="sm" variant="destructive" disabled={reason.length < 10} onClick={() => decide("REJECTED")}>Reject</Button></div>{message && <p role="status" className="text-xs text-muted-foreground">{message}</p>}</div>;
}

export function TreatmentActionUpdate({ actionId, currentStatus }: { actionId: string; currentStatus: string }) {
  return <ActionForm action={(data) => updateTreatmentAction(actionId, String(data.get("status")) as never, String(data.get("note") ?? ""))} submitLabel="Update action"><SelectField name="status" label="Status" options={["NOT_STARTED", "IN_PROGRESS", "BLOCKED", "COMPLETED", "CANCELLED"].map((value) => ({ value, label: value.replaceAll("_", " ") }))}/><Field name="note" label={currentStatus === "COMPLETED" ? "History note" : "Completion or status note"}/></ActionForm>;
}

export function OverdueEscalationButton() {
  const router = useRouter(); const [message, setMessage] = useState("");
  return <div><Button variant="outline" onClick={async () => { const result = await escalateOverdueActions(); setMessage(`${result.count ?? 0} overdue action(s) escalated.`); router.refresh(); }}>Run overdue escalation</Button>{message && <p role="status" className="mt-2 text-xs text-muted-foreground">{message}</p>}</div>;
}

export function ControlProfileForm({ controls, users }: { controls: Option[]; users: Option[] }) {
  return <ActionForm action={(data) => upsertControlProfile({ frameworkControlId: data.get("frameworkControlId"), ownerId: data.get("ownerId") || undefined, implementationStatus: data.get("implementationStatus"), effectiveness: data.get("effectiveness"), frequency: data.get("frequency") || undefined, notes: data.get("notes") || undefined })} submitLabel="Save control profile"><SelectField name="frameworkControlId" label="Enabled framework control" options={controls}/><SelectField name="ownerId" label="Control owner" options={users}/><SelectField name="implementationStatus" label="Implementation state" options={["NOT_STARTED", "IN_PROGRESS", "IMPLEMENTED", "NOT_APPLICABLE"].map((value) => ({ value, label: value.replaceAll("_", " ") }))}/><SelectField name="effectiveness" label="Effectiveness" options={["NOT_ASSESSED", "INEFFECTIVE", "PARTIALLY_EFFECTIVE", "EFFECTIVE"].map((value) => ({ value, label: value.replaceAll("_", " ") }))}/><Field name="frequency" label="Testing frequency"/><Field name="notes" label="Control notes" required={false}/></ActionForm>;
}

export function ControlTestForm({ profiles, evidence }: { profiles: Option[]; evidence: Option[] }) {
  return <ActionForm action={(data) => recordControlTest({ controlProfileId: data.get("controlProfileId"), testDate: data.get("testDate"), result: data.get("result"), notes: data.get("notes") || undefined, evidenceId: data.get("evidenceId") || undefined })} submitLabel="Record test result"><SelectField name="controlProfileId" label="Control profile" options={profiles}/><Field name="testDate" label="Test date" type="date"/><SelectField name="result" label="Result" options={["PASS", "PARTIAL", "FAIL", "NOT_TESTED"].map((value) => ({ value, label: value }))}/><SelectField name="evidenceId" label="Evidence" options={evidence} required={false}/><Field name="notes" label="Test notes"/></ActionForm>;
}

export function ReviewForm({ risks }: { risks: Option[] }) {
  return <ActionForm action={(data) => recordRiskReview({ riskId: data.get("riskId"), scheduledFor: data.get("scheduledFor"), outcome: data.get("outcome"), notes: data.get("notes"), nextReviewDate: data.get("nextReviewDate") })} submitLabel="Record review outcome"><SelectField name="riskId" label="Risk" options={risks}/><Field name="scheduledFor" label="Scheduled review date" type="date"/><SelectField name="outcome" label="Outcome" options={["CONTINUE", "REASSESS", "CLOSE", "ESCALATE"].map((value) => ({ value, label: value }))}/><Field name="notes" label="Review rationale"/><Field name="nextReviewDate" label="Next review date" type="date"/></ActionForm>;
}

export function LifecycleTransitionForm({ riskId, options }: { riskId: string; options: string[] }) {
  return <ActionForm action={(data) => transitionRisk(riskId, String(data.get("status")) as never, String(data.get("reason")))} submitLabel="Apply transition"><SelectField name="status" label="Next status" options={options.map((value) => ({ value, label: value.replaceAll("_", " ") }))}/><Field name="reason" label="Transition rationale"/></ActionForm>;
}

export function EvidenceForm({ risks, plans, actions, controls }: { risks: Option[]; plans: Option[]; actions: Option[]; controls: Option[] }) {
  return <ActionForm action={(data) => createEvidence({ title: data.get("title"), description: data.get("description") || undefined, fileName: data.get("fileName") || undefined, mimeType: data.get("mimeType") || undefined, riskId: data.get("riskId") || undefined, treatmentPlanId: data.get("treatmentPlanId") || undefined, treatmentActionId: data.get("treatmentActionId") || undefined, controlProfileId: data.get("controlProfileId") || undefined, accessLevel: data.get("accessLevel"), expiresAt: data.get("expiresAt") || undefined, retentionUntil: data.get("retentionUntil") || undefined })} submitLabel="Register evidence metadata"><Field name="title" label="Evidence title"/><Field name="description" label="Description"/><div className="grid grid-cols-2 gap-3"><Field name="fileName" label="File name" required={false}/><Field name="mimeType" label="MIME type" required={false}/></div><SelectField name="riskId" label="Risk link" options={risks} required={false}/><SelectField name="treatmentPlanId" label="Treatment plan link" options={plans} required={false}/><SelectField name="treatmentActionId" label="Action link" options={actions} required={false}/><SelectField name="controlProfileId" label="Control link" options={controls} required={false}/><SelectField name="accessLevel" label="Access level" options={[{ value: "WORKSPACE", label: "Workspace" }, { value: "RESTRICTED", label: "Restricted" }]}/><div className="grid grid-cols-2 gap-3"><Field name="expiresAt" label="Expires" type="date" required={false}/><Field name="retentionUntil" label="Retain until" type="date" required={false}/></div></ActionForm>;
}