...
Phase 2 governance is available through `/app/governance` and dedicated `/app/assessments`, `/app/treatments`, `/app/controls`, `/app/reviews`, `/app/evidence`, `/app/governance/scoring-policy`, and `/app/operations/jobs` workspaces. Assessments support revision, submission, four-eyes approval/rejection, and separation of duties; treatments support plans, actions, completion, and overdue escalation; controls support profiles and test history; reviews record outcomes, recurring review schedules (1–24 months), and first-class reassessment requests. The database-backed permission matrix covers all 5 roles (`OWNER`, `RISK_MANAGER`, `ASSESSOR`, `VIEWER`, `AUDITOR`), cross-tenant isolation, soft-deleted records, and single-use token replay protection. The shared `uiCapabilities` contract ensures all UI forms present disabled states with clear accessible reasons when unauthorized. Scoring policies support versioned custom bands while preserving default 1–5 arithmetic, and taxonomy context (business unit, objective, risk source, regulatory domain) is resolved to human-readable names across registers, details, analytics, and exports.
