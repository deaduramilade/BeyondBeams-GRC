...
Phase 3 compliance and framework governance is delivered across `/app/frameworks`, `/app/controls`, and `/app/risks`. Seeded catalogues (ISO 27001, NIST CSF 2.0, SOC 2, HIPAA Security Rule, Fintech/Payments) carry explicit sources, versions, publication dates, and traceable version changelogs. Mappings support structured applicability decisions (`APPLICABLE`, `PARTIALLY_APPLICABLE`, `NOT_APPLICABLE`) with decision rationale and audit trails. Enriched gap analysis provides visual coverage, unmapped controls, unmapped risks, and high residual exposure alerts, exportable as interactive views, board-ready PDFs, and Excel (.xlsx) workbooks. Human-readable taxonomy context (Business Unit, Objective, Risk Source, Regulatory Domain) is resolved everywhere across register columns, filters, detail views, analytics, and CSV/XLSX/PDF exports. The homepage features product quantity stacking ($1\times$ to $99\times$) for paid tiers and updated member capacities. All catalogues and mappings remain governance aids, not legal advice or official certification.
