...
export default async function RiskDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    ...
    <div className="grid items-start gap-5 xl:grid-cols-[1fr_360px]"><div className="space-y-5">
      ...
      <Card><CardHeader><h2 className="text-sm font-bold">Mapped framework controls</h2><p className="mt-1 text-xs text-muted-foreground">Controls from this organisation’s enabled frameworks that treat, evidence, or govern the risk.</p></CardHeader><CardContent><RiskFrameworkMappings riskId={risk.id} mappings={risk.frameworkMappings} controls={controls} editable={capabilities["mapping:manage"]}/></CardContent></Card>
