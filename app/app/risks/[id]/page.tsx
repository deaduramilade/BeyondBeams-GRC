...
import { Calendar, Edit3, ExternalLink, UserRound } from "lucide-react";
...
export default async function RiskDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [risk, controls] = await Promise.all([
    db.risk.findFirst({ where: { id, tenantId: session.user.tenantId, deletedAt: null }, include: { owner: true, complianceLinks: { include: { reference: true } }, frameworkMappings: { include: { frameworkControl: { include: { framework: true } } }, orderBy: { mappedAt: "asc" } }, auditEvents: { include: { actor: true }, orderBy: { createdAt: "desc" } } } }),
    enabledControls(session.user.tenantId),
  ]);
  if (!risk) notFound();
  const editable = writeRoles.includes(session.user.role);
  return <>
    ...
    <div className="grid items-start gap-5 xl:grid-cols-[1fr_360px]"><div className="space-y-5">
      ...
      <Card><CardHeader><h2 className="text-sm font-bold">Mapped framework controls</h2><p className="mt-1 text-xs text-muted-foreground">Controls from this organisation’s enabled frameworks that treat, evidence, or govern the risk.</p></CardHeader><CardContent><RiskFrameworkMappings riskId={risk.id} mappings={risk.frameworkMappings} controls={controls} editable={editable}/></CardContent></Card>
      <Card><CardHeader><h2 className="text-sm font-bold">Triggered obligations & regulatory guidance</h2><p className="mt-1 text-xs text-muted-foreground">Automatically linked from category and risk-statement context. Confirm applicability against the official source.</p></CardHeader><CardContent className="divide-y">{risk.complianceLinks.map((link) => <article key={link.id} className="py-5 first:pt-0"><div className="flex justify-between gap-3"><div><p className="text-[10px] font-bold uppercase text-primary">{link.reference.framework} · {link.reference.reference}</p><h3 className="mt-1 text-sm font-bold">{link.reference.title}</h3></div><a href={link.reference.sourceUrl} target="_blank" rel="noreferrer" className="text-primary"><ExternalLink className="size-4"/><span className="sr-only">Official source</span></a></div><p className="mt-3 text-xs italic leading-6 text-muted-foreground">“{link.reference.excerpt}”</p>{link.reference.metric && <p className="mt-3 rounded-md bg-muted p-3 text-xs"><strong>Metric:</strong> {link.reference.metric}</p>}<p className="mt-2 text-[10px] text-muted-foreground">{link.rationale}</p></article>)}{!risk.complianceLinks.length && <p className="pb-5 text-sm text-muted-foreground">Re-save this risk to generate compliance linkages.</p>}</CardContent></Card>
      <Card><CardHeader><h2 className="text-sm font-bold">Change history</h2></CardHeader><CardContent>{risk.auditEvents.map((event) => <div key={event.id} className="relative border-l pb-6 pl-6 last:pb-0"><span className="absolute -left-1.5 top-0 size-3 rounded-full border-2 border-card bg-primary"/><p className="text-sm font-semibold">{event.summary}</p><p className="mt-1 text-xs text-muted-foreground">{event.actor.name} · {formatDate(event.createdAt)}</p></div>)}</CardContent></Card>
    </div><aside className="space-y-5"><Card><CardHeader><h2 className="text-sm font-bold">Current assessment</h2></CardHeader><CardContent className="space-y-5"><Assessment label="Inherent exposure" likelihood={risk.inherentLikelihood} impact={risk.inherentImpact} score={risk.inherentScore}/><div className="border-t"/><Assessment label="Residual exposure" likelihood={risk.residualLikelihood} impact={risk.residualImpact} score={risk.residualScore}/></CardContent></Card><Card><CardContent className="space-y-4 pt-5"><Meta icon={UserRound} label="Risk owner" value={risk.owner.name}/><Meta icon={Calendar} label="Next review" value={formatDate(risk.nextReviewDate)}/></CardContent></Card></aside></div>
  </>;
}
