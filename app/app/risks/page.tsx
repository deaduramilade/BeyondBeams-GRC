...
export default async function RisksPage() { const session = await requireSession(); const risks = await db.risk.findMany({ where: { tenantId: session.user.tenantId, deletedAt: null }, include: { owner: { select: { name: true } } }, orderBy: { updatedAt: "desc" } }); return <><PageHeader eyebrow="Governance workspace" title="Risk register" description={`${risks.length} active risks across your organisation.`} action={writeRoles.includes(session.user.role)?<Button asChild><Link href="/app/risks/new"><Plus className="size-4"/>Add risk</Link></Button>:undefined}/><RiskTable risks={risks.map(r => ({ ...r, nextReviewDate: r.nextReviewDate.toISOString(), updatedAt: r.updatedAt.toISOString() }))}/></>; }