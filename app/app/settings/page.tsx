...
export default async function Settings() {
  ...
  return <><PageHeader eyebrow="Workspace administration" title="Settings" description="Manage secure workspace controls and notifications."/><div className="grid gap-5"><Card><CardContent className="grid gap-5 pt-5 sm:grid-cols-3">{[["Authentication",user.mfaEnabled?"Password and authenticator MFA":"Credentials enabled"],["Data boundary","All records scoped to session tenant"],["Plan",`${tenant.plan} workspace`]].map(([label,value])=><div key={label}><p className="text-xs font-bold">{label}</p><p className="mt-2 text-xs text-muted-foreground">{value}</p></div>)}</CardContent></Card><MfaSettings enabled={user.mfaEnabled} eligible={[Role.OWNER,Role.RISK_MANAGER].includes(session.user.role)}/><NotificationSettings user={user} tenant={tenant} owner={session.user.role===Role.OWNER}/><Card><CardHeader><div><h2 className="text-sm font-bold">Email a report</h2><p className="mt-1 text-xs text-muted-foreground">Generate a quota-controlled export and send a single-use link that expires after 24 hours.</p></div></CardHeader><CardContent><EmailReportForm email={user.email}/></CardContent></Card></div></>;
}