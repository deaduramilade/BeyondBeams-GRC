...
export async function createRisk(input: RiskInput): Promise<{ error: string } | { success: true; id: string }> {
  ...
  await audit(session.user.tenantId, session.user.id, "CREATE", risk.id, `Created ${reference}`); revalidatePath("/app"); return { success: true, id: risk.id };
}
export async function updateRisk(id: string, input: RiskInput) {
  ...
  await db.risk.update({ where: { id }, data: { ...data, inherentScore: data.inherentLikelihood * data.inherentImpact, residualScore: data.residualLikelihood && data.residualImpact ? data.residualLikelihood * data.residualImpact : null } });
  await audit(session.user.tenantId, session.user.id, "UPDATE", id, `Updated ${existing.reference}`); revalidatePath("/app"); revalidatePath(`/app/risks/${id}`); return { success: true };
}
