...
export async function createRisk(input: RiskInput): Promise<{ error: string } | { success: true; id: string }> {
  ...
  try {
    const risk = await db.$transaction(async (tx: Prisma.TransactionClient) => {
      const [owner, taxonomy] = await Promise.all([
        tx.user.findFirst({ where: { id: data.ownerId, tenantId: session.user.tenantId } }),
        tx.taxonomyItem.findMany({ where: { tenantId: session.user.tenantId, active: true, id: { in: [data.businessUnitId, data.objectiveId, data.riskSourceId, data.regulatoryDomainId].filter((value): value is string => Boolean(value)) } }, select: { id: true } }),
      ]);
      if (!owner) throw new Error("Risk owner is not a member of this workspace.");
      if (taxonomy.length !== [data.businessUnitId, data.objectiveId, data.riskSourceId, data.regulatoryDomainId].filter(Boolean).length) throw new Error("Every organisational context value must belong to this workspace.");
...
export async function updateRisk(id: string, input: RiskInput, expectedVersion: number) {
  ...
  const [existing, owner, taxonomy] = await Promise.all([db.risk.findFirst({ where: { id, tenantId: session.user.tenantId, deletedAt: null } }), db.user.findFirst({ where: { id: data.ownerId, tenantId: session.user.tenantId } }), db.taxonomyItem.findMany({ where: { tenantId: session.user.tenantId, active: true, id: { in: [data.businessUnitId, data.objectiveId, data.riskSourceId, data.regulatoryDomainId].filter((value): value is string => Boolean(value)) } }, select: { id: true } })]); if (!existing) return { error: "Risk not found" }; if (!owner) return { error: "Risk owner is not a member of this workspace." }; if (taxonomy.length !== [data.businessUnitId, data.objectiveId, data.riskSourceId, data.regulatoryDomainId].filter(Boolean).length) return { error: "Every organisational context value must belong to this workspace." };
