...
export async function POST(request: Request) {
  ...
  await linkComplianceToRisk(risk); await db.auditEvent.create({ data: { tenantId: session.user.tenantId, riskId: risk.id, actorId: session.user.id, action: "CREATE", entityId: risk.id, summary: `Created ${risk.reference}` } });
