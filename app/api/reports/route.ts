...
import { auditExportRoles, auditCsv, auditPdf, auditWorkbook, boardPdf, gapAnalysisPdf, gapAnalysisWorkbook, exportAllowance, riskCsv, riskPdf, riskWorkbook } from "@/lib/reporting";
...
export async function GET(request: Request) {
  ...
  if (type === "BOARD_REPORT" && format !== "PDF") return NextResponse.json({ error: "Board report is available only as PDF." }, { status: 400 });
  if (type === "GAP_ANALYSIS" && !["PDF", "XLSX"].includes(format)) return NextResponse.json({ error: "Gap analysis report is available as PDF or Excel (XLSX)." }, { status: 400 });
  ...
  const fileName = type === "BOARD_REPORT" ? "board-risk-report.pdf" : type === "GAP_ANALYSIS" ? `framework-gap-analysis.${format === "XLSX" ? "xlsx" : "pdf"}` : `${type.toLowerCase().replaceAll("_", "-")}.${format === "XLSX" ? "xlsx" : format === "PDF" ? "pdf" : "csv"}`;
  ...
  try {
    const body = type === "BOARD_REPORT"
      ? await boardPdf(session.user.tenantId)
      : type === "GAP_ANALYSIS"
      ? format === "XLSX" ? await gapAnalysisWorkbook(session.user.tenantId) : await gapAnalysisPdf(session.user.tenantId)
      : type === "AUDIT_TRAIL"
      ? format === "PDF" ? await auditPdf(session.user.tenantId, params.get("from") ?? undefined, params.get("to") ?? undefined) : format === "XLSX" ? await auditWorkbook(session.user.tenantId, params.get("from") ?? undefined, params.get("to") ?? undefined) : Buffer.from(await auditCsv(session.user.tenantId, params.get("from") ?? undefined, params.get("to") ?? undefined))
      : format === "PDF" ? await riskPdf(session.user.tenantId) : format === "XLSX" ? await riskWorkbook(session.user.tenantId) : Buffer.from(await riskCsv(session.user.tenantId));
    ...
    return new NextResponse(new Uint8Array(body), { headers: { "Content-Type": type === "BOARD_REPORT" || (type === "GAP_ANALYSIS" && format === "PDF") || format === "PDF" ? "application/pdf" : format === "XLSX" ? "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" : "text/csv; charset=utf-8", "Content-Disposition": `attachment; filename="${fileName.replace(/\.(csv|xlsx)$/i, format === "PDF" ? ".pdf" : (format === "XLSX" ? ".xlsx" : ".csv"))}"`, "Cache-Control": "no-store" } });
...
export async function POST(request: Request) {
  ...
  try {
    const body = type === "BOARD_REPORT"
      ...
      ? format === "XLSX" ? await gapAnalysisWorkbook(session.user.tenantId) : await gapAnalysisPdf(session.user.tenantId)
      : type === "AUDIT_TRAIL"
      ? format === "PDF" ? await auditPdf(session.user.tenantId, from, to) : format === "XLSX" ? await auditWorkbook(session.user.tenantId, from, to) : Buffer.from(await auditCsv(session.user.tenantId, from, to))
      : format === "PDF" ? await riskPdf(session.user.tenantId) : format === "XLSX" ? await riskWorkbook(session.user.tenantId) : Buffer.from(await riskCsv(session.user.tenantId));
